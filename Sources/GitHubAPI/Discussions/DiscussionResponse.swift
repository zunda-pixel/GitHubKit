...
struct DiscussionResponse: Decodable, Sendable {
  ...
  init(from decoder: any Decoder) throws {
    ...
    let repositoryContainer = try dataContainer.nestedContainer(
...
struct DiscussionsResponse: Decodable, Sendable {
  ...
  init(from decoder: any Decoder) throws {
    ...
    let repositoryContainer = try dataContainer.nestedContainer(
      keyedBy: CodingKeys.self, forKey: .repository)
    let discussionsContainer = try repositoryContainer.nestedContainer(
      keyedBy: CodingKeys.self, forKey: .discussions)

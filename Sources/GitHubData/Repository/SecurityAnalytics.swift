  ...
  public init(from decoder: any Decoder) throws {
    ...
    do {
      let nestedContainer = try? container.nestedContainer(
        keyedBy: StatusCodingKeys.self, forKey: .advancedSecurity)
      self.advancedSecurity = try nestedContainer?.decode(Status.self, forKey: .status) == .enabled
    }
    do {
      let nestedContainer = try container.nestedContainer(
        keyedBy: StatusCodingKeys.self, forKey: .dependabotSecurityUpdates)
      self.dependabotSecurityUpdates =
        try nestedContainer.decode(Status.self, forKey: .status) == .enabled
    }
    do {
      let nestedContainer = try container.nestedContainer(
        keyedBy: StatusCodingKeys.self, forKey: .secretScanning)
      self.secretScanning = try nestedContainer.decode(Status.self, forKey: .status) == .enabled
    }
    do {
      let nestedContainer = try container.nestedContainer(
        keyedBy: StatusCodingKeys.self, forKey: .secretScanningPushProtection)
      self.secretScanningPushProtection =
        try nestedContainer.decode(Status.self, forKey: .status) == .enabled
    }
  }

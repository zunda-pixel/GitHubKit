...
public struct Discussion: Codable, Hashable, Sendable, Identifiable {
  ...
  public init(from decoder: any Decoder) throws {
    ...
    self.activeLockReason = try container.decodeIfPresent(
      ActiveLockReason.self, forKey: .activeLockReason)
    self.authorAssociation = try container.decode(
      AuthorAssociation.self, forKey: .authorAssociation)
    ...
    self.viewerSubscription = try container.decode(
      SubscriptionState.self, forKey: .viewerSubscription)
    self.category = try container.decode(Category.self, forKey: .category)
    let labelsContainer = try container.nestedContainer(
      keyedBy: NestCodingKeys.self, forKey: .labels)
    self.labels = try labelsContainer.decode([Label].self, forKey: .nodes)
    let reactionsContainer = try container.nestedContainer(
      keyedBy: NestCodingKeys.self, forKey: .reactions)
    ...
    let commentsContainer = try container.nestedContainer(
      keyedBy: NestCodingKeys.self, forKey: .commentsCount)
